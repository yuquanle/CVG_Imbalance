...
class View_Gen(nn.Module):
    def __init__(self):
        super(View_Gen, self).__init__()
        self.model = BartForConditionalGeneration.from_pretrained("fnlp/bart-base-chinese",return_dict=True)

    def forward(self, view_source_input_ids, view_source_attention_mask, view_target_input_ids, view_decoder_input_ids):
        output = self.model(input_ids = view_source_input_ids, attention_mask = view_source_attention_mask, labels = view_target_input_ids, decoder_input_ids = view_decoder_input_ids)
